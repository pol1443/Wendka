...
namespace Wedka.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, nazwaUzytkownika = model.nazwaUzytkownika, zdjecieProfilowe = "\\Images\\image.jpg", zalozenie = DateTime.Today, ostatnieLogowanie = DateTime.Now };
        ...
        public ActionResult Podglad(string id)
        {
            ApplicationUser user = new ApplicationUser();
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(ApplicationUser model, HttpPostedFileBase files)
        {
            if (ModelState.IsValid)
            {
                {
                    ...
                    if (user != null)
                    {
                        user.Cytat = model.Cytat;
                        user.opis = model.opis;
                        user.nazwaUzytkownika = model.nazwaUzytkownika;
                        if (files != null)
                        {
                            string nazwa = Path.GetFileNameWithoutExtension(files.FileName);
                            string roz = Path.GetExtension(files.FileName);
                            string data = DateTime.Now.ToString("yyyyMMddHHmmss");
                            nazwa += data;
                            nazwa += roz;
                            nazwa = Path.Combine(Server.MapPath("~/Images/"), nazwa);
                            files.SaveAs(nazwa);
                            var userDB = _context.Users.Single(c => c.Id == user.Id);
                            userDB.zdjecieProfilowe = nazwa;
                            TryUpdateModel(userDB);
                            _context.SaveChanges();
                        }
                        else
                        {
                            user.zdjecieProfilowe = model.zdjecieProfilowe;
                        }
                    }
        ...
        public ActionResult MojePosty(string id)
        {
            id = User.Identity.GetUserId();
            List<PostModel> post = _context.Post.Where(x => x.UserId.ToString() == id).ToList();
            return View(post);
        }

        [HttpPost]
        public ActionResult EditPost(ApplicationUser model, HttpPostedFileBase files)
        {
            if (ModelState.IsValid)
            {
                {
